...
//        faceunity.fuSetDefaultOrientation((360 - mInputImageOrientation) / 90);//设置多脸，识别人脸默认方向，能够提高首次识别的速度
    ...
    public int drawFrame(final byte[] img, final int textureId, final int w, final int h, final int orientation) {
        if (this.orientation == -1) {
            this.orientation = orientation;
        }
        if (this.orientation != orientation) {
            this.orientation = orientation;
            onCameraChange(mCurrentCameraType == Camera.CameraInfo.CAMERA_FACING_FRONT ? Camera.CameraInfo.CAMERA_FACING_BACK : Camera.CameraInfo.CAMERA_FACING_FRONT, 0);
        }
        prepareDrawFrame();
        return faceunity.fuDualInputToTexture(img, textureId, mInputTextureType, w, h, mFrameId++, mItemsArray);
    }
